module Impl = (T: {type t;}) => {
  ...
  [@bs.get] external nodeName : T.t => string = "";
  /* nodePrincipal */
  ...
  [@bs.set] external setNodeValue : (T.t, Js.null(string)) => unit =
    "nodeValue" /* temporarily removed to reduce codegen size */; /* let setNodeValue : T.t => option string => unit = fun self value => setNodeValue self (Js.Null.from_opt value); */
  /* Not supported yet
  ...
  [@bs.send.pipe : T.t] external cloneNodeDeep : ([@bs.as {json|true|json}] _) => Dom.node =
    "cloneNode";
  [@bs.send.pipe : T.t] external compareDocumentPosition : Dom.node_like('a) => int =
  ...
  [@bs.send.pipe : T.t]
  external getRootNodeComposed : ([@bs.as {json|{ "composed": true }|json}] _) => Dom.node =
    "getRootNode";
  ...
  external lookupNamespaceURI : string => option(string) =
    "";
  [@bs.send.pipe : T.t] [@bs.return null_to_opt]
  external lookupDefaultNamespaceURI : ([@bs.as {json|null|json}] _) => option(string) =
    "lookupNamespaceURI";
