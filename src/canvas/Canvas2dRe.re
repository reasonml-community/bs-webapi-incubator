...
module type LineCapType = {type t = pri string; let butt: t; let round: t; let square: t;};
...
module type LineJoinType = {type t = pri string; let round: t; let bevel: t; let miter: t;};
...
[@bs.send.pipe : t]
external transform :
  (~m11: float, ~m12: float, ~m21: float, ~m22: float, ~dx: float, ~dy: float) => unit =
  "";
...
external createRadialGradient :
  (~x0: float, ~y0: float, ~x1: float, ~y1: float, ~r0: float, ~r1: float) => gradient =
  "";
...
external bezierCurveTo :
  (~cp1x: float, ~cp1y: float, ~cp2x: float, ~cp2y: float, ~x: float, ~y: float) => unit =
  "";
...
[@bs.send.pipe : t]
external arc :
  (~x: float, ~y: float, ~r: float, ~startAngle: float, ~endAngle: float, ~anticw: Js.boolean) =>
  unit =
  "";

[@bs.send.pipe : t] external rect : (~x: float, ~y: float, ~w: float, ~h: float) => unit = "";

[@bs.send.pipe : t] external isPointInPath : (~x: float, ~y: float) => Js.boolean = "";
...
[@bs.send] external createImageDataCoords : (t, ~width: float, ~height: float) => ImageRe.t =
  "createImageData";

[@bs.send] external createImageDataFromImage : (t, ImageRe.t) => ImageRe.t = "createImageData";

[@bs.send]
external getImageData : (t, ~sx: float, ~sy: float, ~sw: float, ~sh: float) => ImageRe.t =
  "";
